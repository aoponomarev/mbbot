...
- **Управление списком монет**:
  - **Поиск монет**: Поле поиска с автодополнением, поддержка множественного поиска через разделители (запятые, слеши, спецсимволы). Результаты сортируются: полные совпадения по тикеру → тикеры, начинающиеся с запроса → остальные по популярности.
  - **Архив монет**: Структура хранения - массив объектов `{id, symbol, name}`. При архивировании сохраняются ID, тикер (uppercase) и полное название монеты. В выпадающем списке архива показывается только тикер, полное название отображается в tooltip при наведении. Ширина выпадающего списка - по контенту (`width: auto; min-width: fit-content;`).
  - **Контекстное меню**: Доступно по клику на блок иконка+тикер. Операции: перемещение (в начало, вверх, вниз, в конец), архивирование, удаление.
