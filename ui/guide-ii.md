...
- **Интеграция с системой иконок**: Использует `iconsHelper` для загрузки иконок из `icons-mapping.json`
...
- **Централизованная система иконок**: Архитектурный принцип централизованного управления иконками через декларативную конфигурацию (JSON) и стили через CSS переменные, зарутованные от Bootstrap. Соответствия иконок и команд централизованы в JSON файле `ui/config/icons-mapping.json`. Система использует схему "одна иконка → много команд":
...
- **Утилита `ui/utils/icons-helper.js`**: Предоставляет функции для получения иконок из схемы `icons`. Компонент `menu-item` использует эту схему при указании `icon-command`.
