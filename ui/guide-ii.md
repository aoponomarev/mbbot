...
- **Выпадающие меню внутри `<main>`**: Единый стиль для всех выпадающих меню (счетчик монет, поиск, избранное, контекстное меню)
  ...
  - **Стиль фокуса**: Фокус пунктов меню использует общую рутовую переменную `--color-focus-bg` (такой же стиль, как у выбранных строк таблицы), без черной рамки Bootstrap. Подробнее см. раздел "Паттерны универсальных компонентов" в `architect.md`.
...
Подробнее см. раздел "Паттерны универсальных компонентов" в `architect.md`.
...
- **Централизованная система UI элементов**: Архитектурный принцип централизованного управления UI элементами (иконки, кнопки, меню) через декларативную конфигурацию (JSON) и стили через CSS переменные, зарутованные от Bootstrap. Соответствия иконок и команд централизованы в JSON файле `ui/config/ui-element-mapping.json`. Система использует схему "одна иконка → много команд":
  - **Схема `icons` (одна иконка → много команд)**: Позволяет одной иконке соответствовать нескольким командам. Например, иконка `fas fa-star` используется для команд `favorite`, `add-to-favorites`, `remove-from-favorites`. Это упрощает управление иконками, которые используются в разных контекстах с разными командами. Даже если у иконки только одна команда, она все равно хранится в этой схеме для единообразия.
  - **Структура команд**: Каждая команда содержит `label` (текст для отображения в UI) и `tooltip` (подробное описание для всплывающей подсказки). Для индикаторов также используется `label` для короткого текста в UI.
  - **Индикаторы**: Индикаторы (статусы и навигационные указатели) также хранятся в схеме `icons` как команды с `category: "indicators"`.
- **Стили иконок**: Определяются через CSS переменные в `ui/styles/icons.css` и зарутованы от Bootstrap переменных для консистентности.
- **Утилита `ui/utils/ui-element-helper.js`**: Предоставляет функции для получения конфигурации UI элементов из схемы `icons` (`getIconLabel`, `getIconTooltip`, `getIndicatorLabel`, `getIndicatorTooltip`). Компоненты `button` и `menu-item` используют эту схему при указании `icon-command`. Система поддерживает раздельные tooltips для основной части (иконка + текст) и индикатора, что обеспечивает более точное описание функциональности. Для SVG-иконок предоставляет функции `isSVGIcon()` и `getSVGIconPath()` для определения и загрузки SVG-файлов.
- **SVG-иконки**: Хранятся в `ui/assets/icons/` с префиксом `icon-` (например, `icon-cross.svg`). Поддерживают CSS переменные для настройки цвета и толщины линий. Загружаются через `<object>` тег для поддержки `currentColor` и CSS переменных. Компоненты автоматически определяют SVG-иконки и загружают их через `uiElementHelper.getSVGIconPath()`.
- **Каталог иконок**: Интерактивный каталог `ui/assets/icons-review.html` отображает все иконки проекта в табличном формате, сгруппированные по категориям. Показывает превью иконок, идентификаторы, команды, использование в компонентах, файлы/источники. Поддерживает фильтрацию по типу иконки, категории и поиск. Интегрирован с `ui-element-helper.js` для динамической загрузки данных. Использует единую систему управления review-файлами через `ui/review-manager.js` и `ui/review-styles.css`.
- **Каталог цветов**: Интерактивный каталог `ui/styles/colors-review.html` отображает все цветовые переменные проекта в табличном формате, сгруппированные по категориям. Показывает идентификаторы переменных, превью цветов, HSL, RGB, Hex значения, использование в компонентах. Поддерживает фильтрацию по категории и поиск. Использует единую систему управления review-файлами через `ui/review-manager.js` и `ui/review-styles.css`.
- **Система управления review-файлами**: Единая система для управления документационными review-файлами (каталоги иконок, цветов и т.д.). Включает:
  - **`ui/review-manager.js`**: JavaScript-менеджер, обеспечивающий хедер с вкладками для переключения между review-файлами, общие утилиты для сортировки и фильтрации таблиц.
...
- **Преимущества**: Единая точка управления иконками, упрощение добавления новых иконок, консистентность стилей, автоматическая адаптация к темам. Это архитектурный принцип проекта, аналогичный цветовой модели HSL/HSLA и системе z-index слоев. Подробнее см. раздел "Система иконок" в `architect.md`.
