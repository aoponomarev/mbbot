...
- **⚠️ Архитектурные системы UI/UX**: Проект использует несколько архитектурных систем для обеспечения консистентности и централизованного управления UI элементами:
  - **Цветовая модель HSL/HSLA**: Все цвета определяются через HSL/HSLA переменные в `ui/styles/theme-colors.css`. Подробнее см. раздел "Цветовая модель HSL/HSLA" в `docs/guide-ii.md`.
  - **Система z-index слоев**: Все значения `z-index` определяются через CSS переменные в `ui/styles/z-index.css` и связаны с Bootstrap z-index переменными. Подробнее см. раздел "Система z-index слоев" в `docs/guide-ii.md`.
  - **Система UI элементов (ui-element-helper)**: Централизованное управление UI элементами (иконки, кнопки, меню) через декларативную конфигурацию (JSON) и стили через CSS переменные. Подробнее см. раздел "Система иконок" в `docs/guide-ii.md`.
  - **Маркировка контейнеров**: Система автоматической маркировки контейнеров через CSS классы `avto-{hash}` для навигации в коде. Подробнее см. раздел "Маркировка контейнеров" в `docs/guide-ii.md`.
- **⚠️ Стандарты UI/UX**: Детальные правила стилизации, компонентов, паттернов взаимодействия описаны в `docs/guide-ii.md`. В `architect.md` остаются только архитектурные принципы и паттерны.
...
**Детали реализации**: См. раздел "Маркировка контейнеров" в `docs/guide-ii.md`.
...
**Детали реализации**: См. раздел "Система z-index слоев" в `docs/guide-ii.md`.
...
**Детали реализации**: См. раздел "Система иконок" в `docs/guide-ii.md`.
...
**Детали реализации**: См. раздел "Паттерн создания хедеров карточек" в `docs/guide-ii.md`.
...
**Детали реализации**: См. раздел "Парадигма переключения представлений" в `docs/guide-ii.md`.
...
**Детали реализации**: См. раздел "Управление видимостью колонок таблицы через вкладки" в `docs/guide-ii.md`.
...
**Связь с планом миграции**: См. файл `docs/migration-plan.md` для подробного плана миграции и текущего статуса.
...
1. **Изменение интервалов времени**:
   ...
   - **Решение**: См. раздел "Проблема 1" в `docs/migration-plan.md`
...
4. **Независимость MDN от пользовательского горизонта**:
   ...
   - **Решение**: См. раздел "Проблема 4" в `docs/migration-plan.md`
...
> - **UI (интерфейс)** и **UX (паттерны взаимодействий)**: См. разделы "Interface (оформление)" и "Interaction (паттерны UX)" в `docs/guide-ii.md`
...
- `docs/architect.md` содержит архитектурные решения проекта, принципы математической модели, технические стандарты и процессы (протоколирование, синхронизация настроек)
- `docs/guide-ii.md` содержит принципы интерфейса и паттерны взаимодействий
...
│   └── docs/guide-ii.md    # Руководство по интерфейсу и взаимодействию
├── app/               # Точка входа приложения
│   └── app-ui-root.js
├── mm/                # Математические модели
│   └── median/        # Математическая модель "median"
│       ├── docs/mm-median.md  # Документация модели
