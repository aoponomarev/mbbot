...
> **Связь с правилами**: См. раздел "Работа с папкой old_app_not_write" в `.cursorrules`.
...
### ⚠️ Риски при замене интервалов

При замене интервалов дельт изменения цены (например, 60d и 90d → 14d и 200d) **обязательно** провести анализ и пересмотр параметров математической модели:

- весов в функциях расчёта (PRC‑веса, веса для CPT, CD, CGR);
- пороговых значений (thresholds);
- коэффициентов адаптации;
- временных констант (tau, gamma и др.).

Без переоценки этих параметров результаты модели могут стать несопоставимыми со старыми конфигурациями.
...
**Связанные файлы**: `ui/api/coins-manager.js` (методы `getCD()`, `getCDValue()`, `getCDTooltip()`), `docs/mm-median.md` (раздел "Особенности реализации")
